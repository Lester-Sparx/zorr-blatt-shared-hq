...
def _health_module():
    try:
        ...
        pytest.fail("zb_local_controller.daemon_health is missing", pytrace=False)
    return daemon_health


def test_config_sha256_hashes_raw_config_bytes(tmp_path):
    daemon_health = _health_module()
    path = tmp_path / "config.json"
    path.write_bytes(b"{}\n")
    assert daemon_health.config_sha256(path) == hashlib.sha256(b"{}\n").hexdigest()
...
def test_health_schema_and_cycle_summary(tmp_path):
    ...
    writer = daemon_health.DaemonHealthWriter(tmp_path / "runtime", "Lester-Sparx/zorr-blatt-shared-hq", config, 15.0, 1234, "11111111-1111-1111-1111-111111111111")
...
def test_rotating_logger_has_locked_bounds(tmp_path):
    daemon_health = _health_module()
    logger = daemon_health.configure_daemon_logger(tmp_path, "instance", 4321)
...
def test_health_write_uses_sibling_tmp_and_atomic_replace(tmp_path, monkeypatch):
    ...
    writer = daemon_health.DaemonHealthWriter(runtime, "repo/name", config, 15.0, 7, "instance")
    writer.write("STARTING")
    assert calls == [(runtime / "health.json.tmp", runtime / "health.json")]
    assert not (runtime / "health.json.tmp").exists()


def test_health_never_fabricates_github_task_or_comfyui_state(tmp_path):
    daemon_health = _health_module()
    config = tmp_path / "config.json"
    config.write_text("{}", encoding="utf-8")
    writer = daemon_health.DaemonHealthWriter(tmp_path / "runtime", "repo/name", config, 15.0, 7, "instance")
    writer.write("HEALTHY", {"discovered": 0, "processed": 0, "submitted": 0, "skipped": 0})
    data = json.loads((tmp_path / "runtime" / "health.json").read_text(encoding="utf-8"))
    forbidden = {"taskState", "taskStatus", "githubTaskState", "comfyuiState", "salvadorAvailability"}
