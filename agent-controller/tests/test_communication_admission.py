...
def test_accepts_only_canonical_created_top_level_message(): assert admit_event(env(),777).message_id=='msg-001'
@pytest.mark.parametrize('field,value,code',[('authenticated_actor','Duncan-Sparx-ZB','TRANSPORT_ACTOR_REJECTED'),('pr_number',778,'COMMUNICATION_PR_MISMATCH'),('repository','other/repo','COMMUNICATION_PR_MISMATCH'),('event_type','issue_comment.edited','MESSAGE_PROTOCOL_INVALID'),('top_level',False,'MESSAGE_PROTOCOL_INVALID'),('comment_body','ZB_AGENT_RECEIPT_V1\nX = Y','MESSAGE_PROTOCOL_INVALID'),('comment_body','ZB_OWNER_VIEW_V0\nX = Y','MESSAGE_PROTOCOL_INVALID'),('comment_body','hello','MESSAGE_PROTOCOL_INVALID')])
def test_rejects_noncanonical_envelopes(field,value,code):
    with pytest.raises(AdmissionError) as e: admit_event(env(**{field:value}),777)
