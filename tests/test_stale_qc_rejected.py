...
class StaleQcTest(unittest.TestCase):
    def test_old_qc_cannot_lock_new_revision(self):
        ...
        task, qc = submit_review(task, actor="duncan", kind="QC", result="PASS", report_sha256="C" * 64, roles=ROLES)
        task, arch = submit_review(task, actor="django", kind="ARCHITECTURE", result="ACCEPTED", report_sha256="D" * 64, roles=ROLES)
        task["revision"] = 2
