...
function Get-CanonicalTask {
    return Get-ScheduledTask -TaskName $TaskName -ErrorAction SilentlyContinue
}

function Require-Task {
    $task = Get-CanonicalTask
    if ($null -eq $task) { throw "DAEMON_TASK_NOT_INSTALLED" }
    return $task
}
...
function Stop-CanonicalTaskBounded {
    ...
    if ([string]$task.State -eq "Running") {
        ...
        throw "DAEMON_TASK_STOP_TIMEOUT"
    }
}
...
function Get-StatusConfig([string]$PathValue) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($PathValue)) {
        ...
        if ($null -ne $raw.daemonRuntimeRoot -and -not [string]::IsNullOrWhiteSpace([string]$raw.daemonRuntimeRoot)) {
            $runtimeRoot = [string]$raw.daemonRuntimeRoot
        }
        if ($null -ne $raw.pollIntervalSeconds) {
            $poll = [double]$raw.pollIntervalSeconds
        }
    }
    return @{ RuntimeRoot = $runtimeRoot; PollIntervalSeconds = $poll }
}

function Write-DaemonStatus([string]$PathValue) {
    $task = Get-CanonicalTask
    $registered = "NO"
    $enabled = "NO"
    $taskState = "MISSING"
    ...
    if (Test-Path -LiteralPath $healthPath -PathType Leaf) {
        try {
            ...
            if ($null -ne $health.instanceId -and -not [string]::IsNullOrWhiteSpace([string]$health.instanceId)) {
                $instanceId = [string]$health.instanceId
            }
            if ($null -ne $health.configSha256 -and -not [string]::IsNullOrWhiteSpace([string]$health.configSha256)) {
                $configSha = [string]$health.configSha256
            }
            if ($null -ne $health.heartbeatAtUtc) {
                $heartbeat = [DateTimeOffset]::Parse([string]$health.heartbeatAtUtc).ToUniversalTime()
    ...
    Write-Output "ZB_CONTROLLER_DAEMON_STATUS_V1"
...
switch ($Action) {
    "Install" {
        ...
        Write-Output "ZB_CONTROLLER_DAEMON_INSTALL PASS"
    }
    "Uninstall" {
        Stop-CanonicalTaskBounded
        if ($null -ne (Get-CanonicalTask)) {
            Unregister-ScheduledTask -TaskName $TaskName -Confirm:$false
        }
        Write-Output "ZB_CONTROLLER_DAEMON_UNINSTALL PASS"
    }
    "Status" {
        Write-DaemonStatus $ConfigPath
    }
    "Start" {
        $null = Require-Task
        Start-ScheduledTask -TaskName $TaskName
