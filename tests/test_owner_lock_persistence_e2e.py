    ...
    def test_qc_arch_owner_lock_record_passes_persistence_validator(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            task.update({
                ...
                "builderGitHubLogin": "zb-lester", "artifactSha256": ARTIFACT_SHA,
            ...
            self.assertEqual(validate_transition(base, head, actor="zb-lester", base_sha="1" * 40, head_sha=artifact_commit), "ARTIFACT_REGISTERED")
            ...
            state = self.read(head, "hq/state/HQ_STATE.json")
            task = self.read(head, "hq/tasks/GITHUB_SHARED_HQ.json")
            task, qc = submit_review(task, actor="zb-duncan", kind="QC", result="PASS", report_sha256="C" * 64, roles=ROLES)
            ...
            self.write(head, "hq/reviews/qc/GITHUB_SHARED_HQ/r01/zb-duncan.json", qc)
            self.write(head, "hq/state/HQ_STATE.json", state)
            self.write(head, "hq/tasks/GITHUB_SHARED_HQ.json", task)
            self.dashboard(head, state, task)
            self.assertEqual(validate_transition(base, head, actor="zb-duncan", base_sha="3" * 40, head_sha="4" * 40), "QC_RECORDED")
            ...
            state = self.read(head, "hq/state/HQ_STATE.json")
            task = self.read(head, "hq/tasks/GITHUB_SHARED_HQ.json")
            task, architecture = submit_review(task, actor="zb-django", kind="ARCHITECTURE", result="ACCEPTED", report_sha256="D" * 64, roles=ROLES)
            task["expectedMainCommit"] = "5" * 40
            state.update({
                "mainCommit": "5" * 40,
                "lastTransition": {
                    "kind": "ARCHITECTURE_RECORDED", "actorGitHubLogin": "zb-django",
                    "taskRevision": 1, "candidateCommit": artifact_commit,
                    "artifactSha256": ARTIFACT_SHA, "previousRevision": 1,
                },
            })
            self.write(head, "hq/reviews/architecture/GITHUB_SHARED_HQ/r01/zb-django.json", architecture)
            self.write(head, "hq/state/HQ_STATE.json", state)
            self.write(head, "hq/tasks/GITHUB_SHARED_HQ.json", task)
            self.dashboard(head, state, task)
            self.assertEqual(validate_transition(base, head, actor="zb-django", base_sha="5" * 40, head_sha="6" * 40), "ARCHITECTURE_RECORDED")
            ...
            task, lock = create_owner_lock(
                lock_base_state, task, qc, architecture, actor="zb-owner",
            ...
            state.update({
                "mainCommit": "7" * 40,
                "lastTransition": {
                    "kind": "OWNER_LOCKED", "actorGitHubLogin": "zb-owner",
                    "taskRevision": 1, "candidateCommit": artifact_commit,
                    "artifactSha256": ARTIFACT_SHA, "previousRevision": 1,
                },
            })
            ...
            self.assertEqual(validate_transition(base, head, actor="zb-owner", base_sha="7" * 40, head_sha="8" * 40), "OWNER_LOCKED")
