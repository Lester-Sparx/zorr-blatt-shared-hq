...
def test_list_task_issues_uses_current_legal_task_and_comments():
    runner=FakeRunner([cp(out=json.dumps([{"number":92,"title":"x","body":VALID_BODY,"comments":[{"id":"IC_kwDOA1","body":"note"}]}]))])
    gh=BridgeGitHubCLI("Lester-Sparx/zorr-blatt-shared-hq",runner=runner)
    issues=gh.list_task_issues()
    assert issues[0].number==92 and issues[0].comments[0].id=="IC_kwDOA1"
    args,kwargs=runner.calls[0]
    assert args[:3]==["gh","issue","list"]
    assert args[args.index("--state")+1]=="open"
    assert args[args.index("--search")+1]=="ZB_AGENT_TASK_V0"
    assert args[args.index("--limit")+1]=="1000"
    assert args[args.index("--json")+1]=="number,title,body,comments"
    assert kwargs["capture_output"] is True and kwargs["text"] is True and kwargs["shell"] is False

def test_historical_canon_task_kind_is_filtered_not_reintroduced():
    old=VALID_BODY.replace("PRODUCTION_IMAGE_EDIT","CANON_REFERENCE_EDIT")
    runner=FakeRunner([cp(out=json.dumps([{"number":92,"title":"x","body":old,"comments":[]}]))])
    assert BridgeGitHubCLI("o/r",runner=runner).list_task_issues()==()

def test_invalid_task_contract_is_filtered():
    runner=FakeRunner([cp(out=json.dumps([{"number":1,"title":"x","body":"ZB_AGENT_TASK_V0","comments":[]}]))])
    assert BridgeGitHubCLI("o/r",runner=runner).list_task_issues()==()
...
def test_post_reference_event_uses_issue_comment_and_failure_is_stable():
    ok=FakeRunner([cp()]); BridgeGitHubCLI("o/r",runner=ok).post_reference_event(92,"ZB_REFERENCE_EVENT_V1")
    assert ok.calls[0][0][:3]==["gh","issue","comment"]
    bad=FakeRunner([cp(code=1)])
