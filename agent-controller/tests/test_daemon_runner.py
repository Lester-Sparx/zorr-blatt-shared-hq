...
def _runner_types():
    try:
        ...
        pytest.fail("zb_local_controller.daemon_runner is missing", pytrace=False)
    return DaemonPreflightError, DaemonRunner, run_daemon_preflight
...
def test_preflight_authenticates_without_task_discovery(tmp_path):
    _, _, run_daemon_preflight = _runner_types()
...
def test_preflight_probe_write_failure_is_runtime_unwritable(tmp_path, monkeypatch):
    DaemonPreflightError, _, run_daemon_preflight = _runner_types()
    config_path = tmp_path / "config.json"
    config_path.write_text("{}", encoding="utf-8")
...
def test_healthy_cycle_then_graceful_stop():
    _, DaemonRunner, _ = _runner_types()
    health = FakeHealth(); sleeps = []
...
def test_keyboard_interrupt_during_poll_sleep_writes_stopping_and_returns_zero():
    _, DaemonRunner, _ = _runner_types(); health = FakeHealth()
    def interrupted_sleep(_seconds): raise KeyboardInterrupt()
...
def test_transient_github_error_degrades_and_retries():
    _, DaemonRunner, _ = _runner_types(); health = FakeHealth(); sleeps = []
